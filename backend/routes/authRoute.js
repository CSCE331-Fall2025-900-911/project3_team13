...
router.get(
  '/google/callback',
  passport.authenticate('google', {
    failureRedirect: 'http://localhost:5173/not-authorized' //will change later when deploying
  }),
(req, res) => {
  const role = req.user?.role;

  if (role === "manager" || role === "cashier") {
    return res.redirect("http://localhost:5173/layout");
  }

  // Anything else
  return res.redirect("http://localhost:5173/not-authorized");
}
