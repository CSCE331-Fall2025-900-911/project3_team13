...
export function EditorPopup<T extends { id: number; name: string }>(
  ...
  const handleDelete = async () => {
    ...
    try {
      if (tableType === "inventory") {
        await fetch(`https://project3-team13-backend.onrender.com/api/inventory/delete-item?id=${selected.id}`, {
          method: "DELETE"
        });
      } else if (tableType === "employees") {
        await fetch(`https://project3-team13-backend.onrender.com/api/employees/delete-employee?id=${selected.id}`, {
          method: "DELETE"
        });
      } else if (tableType === "menu") {
        await fetch(`https://project3-team13-backend.onrender.com/api/menu/delete-item/${selected.id}`, {
          method: "DELETE"
        });
      }
